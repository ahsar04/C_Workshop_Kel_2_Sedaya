	...
	if(isset($_FILES["foto"]) && $_FILES["foto"]["error"] == 0){
        ...
        if(in_array($filetype, $allowed)){
            // Check whether file exists before uploading it
                ...
                $q = $syntax->view_kon("admin","adm_id='$adm_id'");
                $r = $q->fetch_array();
                $fotoawal=$r['foto'];
            	$file='../public/img/'.$fotoawal;
            	unlink($file);
                move_uploaded_file($_FILES["foto"]["tmp_name"], "../public/img/" . $filename);
				$cek_proses=$syntax->update("mstr_admin","nama='$nama',jk='$jk',tmp_lahir='$tmp_lahir',tgl_lahir='$tgl_lahir',telp='$telp',email='$email',alamat='$alamat',username='$username',password='$pasword',gambar='$foto'","adm_id='$adm_id'");
				if($cek_proses){
	...
	if(isset($_FILES["foto"]) && $_FILES["foto"]["error"] == 0){
        ...
        if(in_array($filetype, $allowed)){
            ...
            if(file_exists("../public/img/" . $filename)){
                ...
                move_uploaded_file($_FILES["foto"]["tmp_name"], "../public/img/" . $filename);
				$cek_proses=$syntax->insert("mstr_admin","adm_id,nama,jk,tmp_lahir,tgl_lahir,telp,email,alamat,username,password,status,gambar",
