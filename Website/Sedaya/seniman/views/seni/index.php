<!-- Content Header (Page header) -->
        ...
        <div class="row">
          <div class="col-12">
            ...
            <h1>Data Postingan</h1>
                <a href="<?=base_url("seniman/index.php?page=postingan/insert");?>"><button class="btn btn-success"><i class="fa fa-plus"></i> Add New</button></a></br>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped table-hover">
                  ...
                  <tbody>
                  <?php
                  $show=$syntax->view("seni join jenis on seni.jns_id=jenis.jns_id where seni.snm_id=$snm_id order by sn_id desc");
                  $n=1;
                  foreach ($show as $r) {
                    $sn_id = $r['sn_id'];
                    $judul = $r['judul'];
                  ...
                  <tr>
                    ...
                    <td class="text-center">
                      <a href="#" data-toggle="modal" data-target="#modal-lg" onclick="tampildata('<?=$sn_id?>','<?=$judul?>','<?=base_url('admin/public/img/seni/'.$gambar)?>','<?=$kategori?>',
                      '<?=$jenis?>','<?=$keterangan?>','<?=$jangkauan?>','<?=$harga?>','<?=$status?>')"><button class="btn btn-secondary"><i class="fa fa-newspaper"></i></button></a>
                      <a href="<?=base_url("seniman/index.php?page=postingan/update&&sn_id=".$sn_id);?>"><button class="btn btn-success"><i class="fa fa-edit"></i></button></a>
                      <a href="<?=base_url('seniman/proses/seni.php?proses=delete&&sn_id='.$sn_id);?>" onclick="return confirm('Hapus postingan?')"><button class="btn btn-danger"><i class="fa fa-trash"></i></button></a>
                    </td>
                  </tr>
                  <?php } ?>
                  </tbody>
            ...
            <div class="modal-dialog modal-lg">
              <div class="modal-content bg-info">
                ...
                <div class="modal-footer justify-content-between">
                  ...
                  <form action="<?=base_url('seniman/proses/seni.php?proses=konfirmasi');?>" method="POST">
    ...
    <!-- /.content -->
    <script>
      function tampildata(sn_id,judul,gambar,kategori,jenis,keterangan,jangkauan,harga,status) {
